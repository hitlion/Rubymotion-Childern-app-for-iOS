class TabletOptionView < UIView
  ...
  def build_view
    frame = CGRectMake(0,0, 0.75 * self.frame.size.width, self.frame.size.height)
    background = UIButton.alloc.initWithFrame(frame)
    background.setImage(UIImage.imageNamed("background_grey_trans.png"), forState:UIControlStateNormal)
    background.addTarget(self, action: "close_options", forControlEvents: UIControlEventTouchDown)
